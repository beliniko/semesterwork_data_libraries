...
def readOrFetchData():
    try:
        with open('bokeh_questions.json', 'r') as file:
            # Lesen des gesamten Inhalts der Datei
            data = file.read()
            # Konvertieren des JSON-Strings in ein Python-Datenobjekt
            bokeh_questions = json.loads(data)
            return bokeh_questions
...
def fetchBokehQuestions():
    ...
    bokeh_questions = stackoverflow.fetch('questions', fromdate=datetime(2023, 1, 1), todate=datetime(2023, 12, 31),
                                          tagged='bokeh')
    # Konvertieren des Python-Datenobjekts in einen JSON-String
    json_data = json.dumps(bokeh_questions, indent=4)
    # Schreiben des JSON-Strings in eine Datei
    with open('bokeh_questions.json', 'w') as file:
        file.write(json_data)
    return bokeh_questions


bokeh_questions = readOrFetchData()

items = bokeh_questions['items']
count_true = 0
for item in items:
